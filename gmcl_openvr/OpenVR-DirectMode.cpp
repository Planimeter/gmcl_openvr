...
void OpenVRDirectMode::Init(IDirect3DDevice9Ex* pActualDevice)
{
	vireio::debugf("OpenVRDirectMode::Init");

	if (initialized) {
		OutputDebugString("OpenVRDirectMode already Init'd\n");
		return;
	}
...
void OpenVRDirectMode::PrePresentEx(IDirect3DSurface9* pDirect3DSurface9 )
{
	if (!vr::VRCompositor())
	{
...
void OpenVRDirectMode::PostPresentEx(IDirect3DSurface9* pDirect3DSurface9 )
{
	...
	vr::EVRCompositorError error = vr::VRCompositor()->Submit(vr::Eye_Left, &leftEyeTexture, &m_sTextureBounds[0]);
